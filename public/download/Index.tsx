...
// 表单验证
function ruleChecked(rule: objTypes) {
	// 修改错误状态
	...
	// 在这开始验证自定义的规则
	let errorRule = rule.rules.find((item: objTypes) => !toRegExp(item.pattern).test(rule.value));
	if (errorRule) {
...
// 问题，描述语，以及图片
const LabelContent = memo((attrs: objTypes) => (
...
function Preview(props: propTypes) {
	...
	// 表单头部内容描述
	const formHeader: headerTypes | objTypes = formData?.header || {};
	// 结束语
	const formEnd: headerTypes | objTypes = formData?.end || {};
	// 当前渲染的列表
	...
	// 验证规则的对象列表
	const [ruleList, setRuleList] = useState<objTypes>({});
	// 提交的数据格式
	...
	const clearContactCallBack = (clearJump: Array<string>) => {
		...
		clearJump.forEach(id => {
			...
			if (formItem.type === 'select') {
				// 找到清除关联选择题下面的逻辑关联列表
				let clearJumpList: Array<string> = formItem.options.flatMap((item: optionTypes) => item.jumpTo);
						...
						<>
							...
							{item.type === 'date' ?
								<div className='date-picker-wrapper'>
									<ScheduleOutlined className='date-icon' />
									{item.tag === 'date' ?
										<DatePicker
